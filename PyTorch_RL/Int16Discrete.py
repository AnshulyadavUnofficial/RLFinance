...
class Int16Discrete(Space[np.int16]):
    r"""A space consisting of finitely many int16 elements.

    This class represents a finite subset of integers: { start, start+1, ..., start+n-1 }.
    """
    ...
    def __init__(
        ...
        assert np.issubdtype(type(n), np.integer), f"n must be int, got {type(n)}"
        assert n > 0, "n (counts) must be positive"
        assert np.issubdtype(type(start), np.integer), f"start must be int, got {type(start)}"

        self.n = np.int16(n)
        self.start = np.int16(start)
        super().__init__((), np.int16, seed)
    ...
    def sample(
        ...
        """Sample an int16 action (supports mask or probability weighting)."""
        if mask is not None and probability is not None:
            raise ValueError("Only one of `mask` or `probability` may be provided")

        elif mask is not None:
            assert isinstance(mask, np.ndarray), f"mask must be ndarray, got {type(mask)}"
            assert mask.dtype == np.int8, f"mask dtype must be int8, got {mask.dtype}"
            assert mask.shape == (self.n,), f"mask shape must be ({int(self.n)},), got {mask.shape}"

            valid_action_mask = mask == 1
            if np.any(valid_action_mask):
                return np.int16(self.start + self.np_random.choice(np.where(valid_action_mask)[0]))
            else:
                return np.int16(self.start)

        elif probability is not None:
            assert isinstance(probability, np.ndarray), f"probability must be ndarray, got {type(probability)}"
            assert probability.dtype == np.float64, f"probability dtype must be float64, got {probability.dtype}"
            assert probability.shape == (self.n,), f"probability shape must be ({int(self.n)},), got {probability.shape}"
            assert np.all((probability >= 0) & (probability <= 1)), f"probability values must be in [0,1]"
            assert np.isclose(np.sum(probability), 1), f"probabilities must sum to 1"

            return np.int16(self.start + self.np_random.choice(np.arange(self.n), p=probability))

        else:
            return np.int16(self.start + self.np_random.integers(self.n))
    ...
    def contains(self, x: Any) -> bool:
        ...
        return bool(self.start <= as_int16 < self.start + self.n)

    def __repr__(self) -> str:
        return f"Int16Discrete({self.n}, start={self.start})" if self.start != 0 else f"Int16Discrete({self.n})"

    def __eq__(self, other: Any) -> bool:
        return isinstance(other, Int16Discrete) and self.n == other.n and self.start == other.start

    def __setstate__(self, state: Iterable[tuple[str, Any]] | Mapping[str, Any]):
        state = dict(state)
        if "start" not in state:
            state["start"] = np.int16(0)
        super().__setstate__(state)

    def to_jsonable(self, sample_n: Sequence[np.int16]) -> list[int]:
        return [int(x) for x in sample_n]

    def from_jsonable(self, sample_n: list[int]) -> list[np.int16]:
        return [np.int16(x) for x in sample_n]
